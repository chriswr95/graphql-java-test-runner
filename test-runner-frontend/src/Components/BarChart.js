...
export default function BarCharts(classesAndBenchmarksState) {
  ...
  const getModeName = (mode) => {
    ...
    return Object.entries(benchmarkModes)
      .filter(([key, value]) => key === mode)
      .map(([first, second]) => second);
  };

  return (
    <Box>
      <Chip sx={{ marginTop: '1.8%', marginBottom: '3.6%', backgroundColor: 'F1F1F1' }} label={getModeName(mode)} />
      <BarChart width={520} height={300} data={benchmarksDataState} layout="vertical">
        <XAxis type="number" />
